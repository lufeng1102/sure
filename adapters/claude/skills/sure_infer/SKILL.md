...
1. 启动运行（执行 pre_start 门控）：调用 `sure_run_start`，入参
   `{ "skill": "sure_infer", "args": "<用户参数>" }`。若用户以 `/sure_infer <参数>` 带入参数就用它；否则用手册 args 示例（如 `model=Qwen__Qwen3-ASR-1.7B datasets=/srv/sure/datasets/group/store/ds_pool/example@v1.0.2`）补齐，或先向用户索取必需参数。
